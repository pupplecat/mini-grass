...
// !!! Ref: from sanctum-solana-utils

// TODO: we can change this to just be `&'slice [S]` and remove the `?Sized` once `&dyn Signer` impls Signer once https://github.com/solana-labs/solana/pull/34984
...
/// Behavior is undefined if slice passed in is not sorted in pubkey order
pub struct SortedSigners<'slice, 'signer, S: ?Sized>(pub &'slice [&'signer S]);

impl<S: ?Sized> SortedSigners<'_, '_, S> {
...
impl<'a, S: Signer + ?Sized> Iterator for SortedSignerIter<'a, '_, '_, S> {
    type Item = &'a S;
...
impl<S: Signer + ?Sized> Signers for SortedSigners<'_, '_, S> {
